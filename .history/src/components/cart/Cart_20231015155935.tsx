...
export default function Cart(props: {
  ...
  const { item, dispatch, index } = props

  const [showModal, setShowModal] = useState<boolean>(false) // muncul ketika menghapsu cart item
  ...
  useEffect(() => {
    if (item?.selected) {
      setChoice(item?.selected)
    }
  }, [])

  const base = 'default'
  const name = item ? item.name : base
  const description = item ? item.description : base
  const price = item ? item.price : base
  const size = item ? item.size : base
  const qty = item ? item.qty : 1
  const indexCart = item ? index : 0
  const image = item ? item.image : base
  ...
  const handlePlus = () => {
    ...
    if (item?.selected) {
      const item = { ...item, qty: newQuantity }
  ...
  const handleChoiceItem = () => {
    // console.log('in handleChoiceItem cart.tsx')
    const item = { ...item, qty: quantity }
    ...
    <>
      ...
      <div className='flex gap-x-2 border-b border-sekunder md:justify-between py-4'>
        <div className='flex flex-col'>
          <label>
            <input
              defaultChecked={item?.selected ? item.selected : false}
              onChange={handleChoiceItem}
              type='checkbox'
