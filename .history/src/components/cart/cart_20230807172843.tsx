...
export default function Cart(props: { items: itemType }) {
  const { items } = props
  console.log(items)
  const name = items ? items.name : 'default name'
  const description = items ? items.description : 'default description'
  const price = items ? items.price : 'default prize'
  const size = items ? items.size : 'default size'
  ...
  return (
    <tr>
      <td className='flex flex-col gap-y-2 items-start'>
        ...
        <input className='checkbox w-[1.5rem] h-[1.5rem]'>x</input>
      </td>
      <td>
