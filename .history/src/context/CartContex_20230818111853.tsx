...
function cartReducer(state: itemType[], action: CartAction): itemType[] {
  switch (action.type) {
    case 'GET':
      ...
      // return JSON.parse(cartGET ? cartGET : '')

      const cart = action.defaultItem ? JSON.parse(action.defaultItem) : []
      return cart
    case 'COOKIES':
      const cartSET = action.items as itemType
      window.localStorage.removeItem('cart')
      window.localStorage.setItem('cart', JSON.stringify(cartSET))
