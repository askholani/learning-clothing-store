...
function cartReducer(state: itemType[], action: CartAction): itemType[] {
  switch (action.type) {
    ...
    case 'ADD':
      ...
      let updatedItem
      const cartItemsSameId = oldItems.filter((item : {id :number})=>item.id === newItems.id)

      if (cartItemsSameId) {
        const existingIndexSameSize = cartItemsSameId.findIndex((item : {size : string})=>item.size ===newItems.size)
        existingCartItem = cartItemsSameId[existingIndexSameSize]
        updatedItem = {...existingCartItem,qty : existingCartItem.qty + newItems.qty}
        updatedItems = [...state]
