...
function cartReducer(state: itemType[], action: CartAction): itemType[] {
  switch (action.type) {
    ...
    case 'REMOVE':
      ...
      const index = action.indexCart as number
      const newItemsRemove = state.splice(index, 1)
      console.log('newItemsRemove', newItemsRemove)
      console.log('state after', state)
      // console.log('itemRemove', itemRemove)
      // const newItemsRemove = state.filter(
      //   (item: { id: number; size: string }) =>
      //     itemRemove.id !== item.id && itemRemove?.size !== item.size,
      // )
      // console.log('newItemsRemove', newItemsRemove)
      return state
  }
