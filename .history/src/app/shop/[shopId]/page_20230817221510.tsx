...
export default function Page() {
  ...
  let images = []
  if (!imageItems.isLoading) {
    images = imageItems.data.result.images
  }
  console.log('images', images)

  let detail: any
  if (!detailItems.isLoading) {
    detail = detailItems.data.result
  }
  console.log('detail', detail)

  let itemProductImages = Object.values(images ? images : {}).map((a) => a)
  itemProductImages.push(detail.image)
      ...
      <Suspense fallback={<Loading />}>
        <section className='grid grid-cols-12 gap-x-8 border-b pb-8'>
          ...
          <div className='col-span-7 flex flex-col'>
            ...
            <h2 className='text-5xl font-bold my-8'>{detail.name}</h2>
            <h3 className='text-xs'>{detail.description}</h3>
            <div className='flex flex-col w-full my-8'>
              <div className='flex justify-between py-4 border-y'>
                ...
                <div className='flex gap-x-4'>
                  ...
                  <h4 className='normal-case font-light'>{detail.stock}</h4>
                </div>
              </div>
              <h3 className='text-end text-2xl'>${detail.price}</h3>
